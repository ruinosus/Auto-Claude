 ...
 * ===============
 *
 * Displays artifacts generated by AI agents, grouped by agent type.
 * Supports search, filtering, and detailed viewing.
 *
 * Now uses ROI Engine API via the API Bridge for artifact data.
 */

import { useState, useEffect, useCallback } from 'react';
import { useTranslation } from 'react-i18next';
import { apiBridge } from '../../../services/api-bridge';
import type { Artifact } from '../../../services/analytics-api';
...
import type { ArtifactTab, ArtifactType } from '../../../services/analytics-api';
  ...
  useEffect(() => {
    async function fetchArtifacts() {
      ...
      try {
        ...
        // Use API Bridge - routes to ROI Engine for artifact data
        ...
        if (hasSearchParams && searchParams.query) {
          // Use search endpoint via API bridge
          const searchResponse = await apiBridge.artifacts.search(
            searchParams.query,
            context,
            {
              artifactType: searchParams.types?.[0],
              limit: 200,
            }
          );
          // Map search results to Artifact format
          const results = 'results' in searchResponse ? searchResponse.results : [];
          ...
          // Fetch from local storage via API bridge
          const response = await apiBridge.artifacts.listLocal({
